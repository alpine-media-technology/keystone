...
function register () {
	...
	if (this.get('inherits')) {
		...
		this.model = keystone.mongoose.model(this.key, this.schema, this.plural.replaceAll(' ', '').toLowerCase());
	}
	...
	this.model.on('error', function (err) {
		if (err) {
			console.log(err)
			if (process.env.KEYSTONE_LOG_MODEL_ERRORS === 'true') {
				console.error('Mongoose model \'error\' event fired on \'' + list.key + '\' with error:\n', err.message, err.stack);
			} else {
				debug('Mongoose model \'error\' event fired on \'' + list.key + '\' with error:\n', err.message, err.stack);
			}
		}
	});
	return this;
